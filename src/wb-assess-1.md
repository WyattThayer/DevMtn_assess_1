...
```
Console.log is used for debugging and will not show up for a user since its just a console command return will provide an output for a user not using the console.
```

**_What's the difference between defining and calling a function?_**

```
defining is creating the code inside the function. calling is using the function later in your code.
```
...
```
words placed in "" it tells javascript that there is not code in here just display the information.
```

**_Number_**

```
javascript recognizes this as just a number (i.e. 1,2,3) and it doesn't need anything extra to be displayed as such
```

**_Boolean_**

```
true or false it takes in data and decides if the statement is one of the two options
```

**_Array_**

```
a collection of all the primitive types stored inside of square braces []
```
